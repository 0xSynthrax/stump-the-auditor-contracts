...
contract LendingTest is BaseTest {
    ...
    function setUp() public override {
        ...
        _listReserve(address(usdc), 8_000, 1_000, true, true);
        _listReserve(address(weth), 7_500, 1_000, true, true);
        _listReserve(address(wbtc), 7_000, 1_000, true, true);
    ...
    //     vm.startPrank(owner);
    //     lending.setCloseFactor(10_000);
    //     _listReserve(address(usd6), 8_000, 1_000, true, true);
    ...
    //     vm.expectEmit(true, false, false, true);
    //     emit ReserveParamsUpdated(address(usdc), 7_000, 800);
    //     vm.prank(owner);
    //     lending.setReserveParams(address(usdc), 7_000, 800);
    ...
    //
    //  1. CURRENT VERSION (before bug insertion):
    //     forge test --match-test testPartialLiquidationWorsensBorrowerHealthFactor -vvv
    //     Expected result: PASS — HF improves after each liquidation pass, confirming
    //     the existing key_ratio guard (LT * (1 + bonus) < 1) works correctly.
    //
    //  2. AFTER BUG INSERTION (LIQUIDATION_THRESHOLD_BPS=9_000, LIQUIDATION_BONUS_BPS=2_000):
    //     - Update setUp() to use the new listReserve/setReserveParams signatures
    //       (liquidationThresholdBps and liquidationBonusBps params removed).
    //     - Re-run the same test.
    //     Expected result: FAIL — HF worsens after each pass because
    //     key_ratio = 0.9 * 1.2 = 1.08 >= 1, proving the vulnerability is active.
    //
    //  NOTE ON setUp() AFTER BUG INSERTION:
    //  The _listReserve() helper and any direct lending.setReserveParams() calls that
    //  currently pass liquidationThresholdBps / liquidationBonusBps must have those
    //  two arguments removed to match the new function signatures.
    // ============================================================

    // ============================================================
    //  MATHEMATICAL EXPECTATION
    //
    //  Position setup: Bob supplies 1 WETH, borrows 1_000 USDC.
    //  WETH price dropped to $1_100 to create an undercollateralised position.
    //  Close factor: 50% → max repay per call = 500 USDC.
    //
    //  CURRENT VERSION  (LT=0.80, bonus=0.05, key_ratio=0.84):
    //    HF_before  = 1100 * 0.80 / 1000 = 0.88
    //    After pass (repay $500, seize $525):
    //      C' = $1100 - $525 = $575,  D' = $500
    //      HF' = 575 * 0.80 / 500   = 0.92  ← IMPROVED ✓
    //
    //  BUGGY VERSION  (LT=0.90, bonus=0.20, key_ratio=1.08):
    //    HF_before  = 1100 * 0.90 / 1000 = 0.99
    //    After pass (repay $500, seize $600):
    //      C' = $1100 - $600 = $500,  D' = $500
    //      HF' = 500 * 0.90 / 500   = 0.90  ← WORSENED ✗
    //
    //  The test asserts assertGt(hfAfter, hfBefore) on every pass.
    //  This assertion holds in the current version and breaks in the buggy version.
    // ============================================================


    function testPartialLiquidationWorsensBorrowerHealthFactor() public {
        // ── Setup ──────────────────────────────────────────
        _supply(alice, usdc, 2_000e6, alice);
        _supply(bob, weth, 1 ether, bob);
        _borrow(bob, usdc, 1_000e6, bob);

        // Drop WETH price to make Bob's position liquidatable.
        // At $1_100:
        //   current version → HF ≈ 0.88  (LT=80%, key_ratio=0.84)
        //   buggy version   → HF ≈ 0.99  (LT=90%, key_ratio=1.08)
        vm.prank(owner);
        oracle.setPrice(address(weth), 1_100e8);

        (,,, uint256 hfBefore) = lending.getUserAccountData(bob);
        assertLt(hfBefore, lending.MIN_HEALTH_FACTOR(), "position must be liquidatable before pass 1");

        // ── Pass 1 ─────────────────────────────────────────
        // Read remaining debt and compute max repay = 50% of it.
        // Pass 1: 50% of 1_000 USDC = 500 USDC.
        (, uint256 debtBefore1) = lending.getUserReserveData(bob, address(usdc));
        uint256 maxRepay1 = debtBefore1 * lending.closeFactorBps() / lending.BPS();

        vm.prank(charlie);
        lending.liquidate(bob, address(weth), address(usdc), maxRepay1);

        (,,, uint256 hfAfterPass1) = lending.getUserAccountData(bob);

        // CORE ASSERTION:
        //   Current version : hfAfterPass1 > hfBefore  → PASS  (HF improved)
        //   Buggy version   : hfAfterPass1 < hfBefore  → FAIL  (HF worsened)
        assertGt(
            hfBefore,
            hfAfterPass1,
            "pass 1: HF must improve after partial liquidation (fails when key_ratio >= 1)"
        );

        // ── Pass 2 ─────────────────────────────────────────
        // Only executes if the position is still liquidatable after pass 1.
        // Re-reads remaining debt so the close factor is applied to the correct base.
        // Pass 2: 50% of remaining ~500 USDC = ~250 USDC.
        if (hfAfterPass1 < lending.MIN_HEALTH_FACTOR()) {
            (, uint256 debtBefore2) = lending.getUserReserveData(bob, address(usdc));
            uint256 maxRepay2 = debtBefore2 * lending.closeFactorBps() / lending.BPS();

            vm.prank(charlie);
            lending.liquidate(bob, address(weth), address(usdc), maxRepay2);

            (,,, uint256 hfAfterPass2) = lending.getUserAccountData(bob);

            assertGt(hfAfterPass1,  hfAfterPass2, "pass 2: HF worsened again (key_ratio >= 1)");
        }
    }
}
